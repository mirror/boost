...
int main(){
  ...
  // create custom special_values parser and formatter objetcs
  ...
  // custome date_generator phrases
  ...
  //  create date_facet and date_input_facet
  ...
  date_period dp(date(2005,3,1), days(31)); // month of march
  first_day_of_the_week_before d_gen(Monday);
  // defaults
  ss.str("");
  ss << dp << "; " << d_gen;
  cout << ss.str() << endl; // "[2005-Mar-01/2005-Mar-31]; Mon before"
  
  // customize everything in the output facet
  ...
  ss.str("from 2005-february-01 to 2005-march-01 exclusive");
  ss >> dp;
  ss.str("wednesday prior to");
  ss >> d_gen;
  ss.str("");
  ss << dp << " is the month of " << dp.begin().month();
  cout << ss.str() << endl;
  ss.str("");
  ss << "The " << d_gen << ' ' << dp.begin() << " is " << d_gen.get_date(dp.begin());
  cout << ss.str() << endl;
