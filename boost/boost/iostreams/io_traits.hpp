...
// 
// Contains metafunctions char_type, category and mode used for deducing the 
// category and mode of a model of Filter or Device.
//
...
//------------------Definitions of char_type, category------------------------//
...
struct member_category { typedef typename T::category type; };
    ...
    template<typename T>
    struct char_type : detail::member_char_type<T> { };
    template<typename Container>
    struct char_type< std::back_insert_iterator<Container> > {
        typedef typename Container::value_type type;
    };
    template<typename T>
    struct char_type< boost::reference_wrapper<T> > 
        : char_type<T> 
        { };
    template<typename T>
    struct category {
        typedef typename  
                select<
                    ...
                    mpl::true_,          lazy< detail::member_category<T> >
                >::type type;     
    };
    template<typename Container>
    struct category< std::back_insert_iterator<Container> > {
        typedef insert_iterator_tag type;
    };
    template<typename Container>
    struct category< const std::back_insert_iterator<Container> > {
        typedef insert_iterator_tag type;
    };
    template<typename T>
    struct category< boost::reference_wrapper<T> > 
        : category<T> 
        { };
    template<typename T>
    struct category< const boost::reference_wrapper<T> > 
        : category<T> 
        { };
    ...
    struct category {
        ...
        typedef typename  
                select<
                    ...
                    mpl::true_,           lazy< detail::member_category<U> >
                >::type type;     
    };
#endif // #ifndef BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION //-----------------//

// Macros for readability.
#define BOOST_IOSTREAMS_CHAR_TYPE(T) typename boost::iostreams::char_type<T>::type
#define BOOST_IOSTREAMS_CATEGORY(T) typename boost::iostreams::category<T>::type
    /**/
...
// 
// Returns an object of type category<T>::type.
// 
template<typename T>
...
template<int N> struct mode_impl;

#define BOOST_IOSTREAMS_MODE_IMPL_SPEC(tag_, id_)                   \
    case_<id_> mode_impl_helper(tag_);                       \
    template<> struct mode_impl<id_> { typedef tag_ type; }; \
    /**/
...
struct mode_id {
    ...
    BOOST_SELECT_BY_SIZE(int, value, detail::mode_impl_helper(category()));
};

} // End namespace detail.

template<typename T> // Borland 5.6.4 requires this circumlocution.
struct mode : detail::mode_impl< detail::mode_id<T>::value > { };
