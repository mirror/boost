#ifndef BOOST_DETAIL_PROPERTY_ACCESSOR_HPP
#define BOOST_DETAIL_PROPERTY_ACCESSOR_HPP
...
namespace boost {
  ...
  namespace detail {
    ...
    // A helper class for constructing a property accessor
...
namespace boost {
  namespace detail {
    ...
    // put()/get()/at() property accessor interface of boost to the
    ...
    struct undefined_property_accessor_category { };

    // ID_PA only used with random access iterators
    template <class PA, class ID_PA = identity_property_accessor>
    struct property_accessor_adaptor {
	  ...
	  typename ct_if<int(id) == WRITABLE_PA, writable_adaptor<PA>,
	    typename ct_if<int(id)==READ_WRITE_PA, read_write_adaptor<PA>,
		  ...
		  typename ct_if<int(id) == OP_BRACKET_PA, 
		    ...
		    undefined_property_accessor_category
		  >::type
	      >::type
	    >::type
	  >::type
    ...
    // This won't work very well with lvalue property accessors.
    // Need to figure out a way to make property_accessor_adaptor
    // work with VC++. -JGS
    template <class PA, class T, class K>
    class pa2dec_proxy {
    public:
...
#endif // BOOST_DETAIL_PROPERTY_ACCESSOR_HPP
