...
namespace boost { namespace numeric { namespace ublas {
    ...
    block_prod (const matrix_expression<E1> &e1,
        ...
        for (size_type i_begin = 0; i_begin < i_size; i_begin += block_size) {
            size_type i_end = i_begin + min BOOST_PREVENT_MACRO_SUBSTITUTION (i_size - i_begin, block_size);
            ...
            for (size_type j_begin = 0; j_begin < j_size; j_begin += block_size) {
                size_type j_end = j_begin + min BOOST_PREVENT_MACRO_SUBSTITUTION (j_size - j_begin, block_size);
#ifdef BOOST_UBLAS_NO_CACHE
        ...
        for (size_type i_begin = 0; i_begin < i_size; i_begin += block_size) {
            ...
            for (size_type j_begin = 0; j_begin < j_size; j_begin += block_size) {
                ...
                for (size_type k_begin = 0; k_begin < k_size; k_begin += block_size) {
                    size_type k_end = k_begin + min BOOST_PREVENT_MACRO_SUBSTITUTION (k_size - k_begin, block_size);
#ifdef BOOST_UBLAS_NO_CACHE
