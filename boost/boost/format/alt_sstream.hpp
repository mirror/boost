...
#include <string>
#include <boost/format/detail/compat_workarounds.hpp> // provides std::basic_streambuf
...
namespace boost {
    namespace io {
        ...
        public:
            ...
            explicit basic_altstringbuf (ios_base::openmode mode = ios_base::in | ios_base::out)
                : putend_(NULL), is_allocated_(false), mode_(mode) {}
            explicit basic_altstringbuf (const string_type& s,
                                         ios_base::openmode mode = ios_base::in | ios_base::out)
                : putend_(NULL), is_allocated_(false), mode_(mode) { dealloc(); str(s); }
            ...
            streamsize pcount() const { return static_cast<streamsize>( pptr() - pbase()) ;}

            // copy buffer to string :
            string_type str() const {return string_type(begin(), size()); }
            string_type cur_str() const { return string_type(begin(), cur_size()); }
        protected:
...
// ---   class basic_oaltstringstream -----------------------------------------------
        ...
        {
            class No_Op { // used as no-op deleter for (not-owner) shared_pointers
            public: 
                template<class T>
            ...
            typedef typename CompatTraits<Tr>::type_for_string    stringTr;
            typedef typename CompatAlloc<Alloc>::type_for_string     stringAlloc;
            typedef typename CompatOStream<std::basic_ostream<Ch, Tr> > ::compatible_type  stream_t;
            typedef boost::base_from_member<boost::shared_ptr<basic_altstringbuf<Ch,Tr, Alloc> > > 
                pbase_type;
        public:
            typedef typename CompatTraits<Tr>::compatible_type traits_type;
            ...
            basic_oaltstringstream(stringbuf_t * buf) 
                ...
                { }
            stringbuf_t * rdbuf() const { return pbase_type::member.get(); }
            ...
            // copy buffer to string :
            string_type str()     const { return rdbuf()->str(); }     // [pbase, epptr[
            string_type cur_str() const { return rdbuf()->cur_str(); } // [pbase, pptr[
        };
